# OSINT-Tools-Bookmarks
This is a collection of web-based tools that I find useful for general cybersecurity related stuff. They're organized in categories and you can download the bookmark html file to add them all to your browser as a bookmark folder
# Tools and Categories:
# E-mail Tools
## Header analysers
* [Analyze my e-mail header](https://mailheader.org/)
* [Google admin toolbox - MessageHeader](https://toolbox.googleapps.com/apps/messageheader/analyzeheader)
* [Message Header Analyser](https://mha.azurewebsites.net/)
## Phishing
* [URL extractor](https://www.convertcsv.com/url-extractor.htm)
* [PhishTank](https://phishtank.com/?)
* [PhishTool](https://www.phishtool.com/)
## Leaks
* [Have I Been Pwned?](https://haveibeenpwned.com/)
* [DeHashed](https://www.dehashed.com/)
# IP and Domain Tools
## IP lookup
* [What is my IP.com.br](https://whatismyip.com.br/)
* [AbuseIPDB - IP address abuse reports](https://www.abuseipdb.com/)
* [ipinfo](https://ipinfo.io/)
* [IP location](https://www.iplocation.net/)
## DNS
* [MX Lookup Tool - Check your DNS MX Records online - MxToolbox](https://mxtoolbox.com/)
* [dnslytics](https://dnslytics.com/)
* [DomainTools](https://whois.domaintools.com/)
* [Hurricane Electric Internet Services - Internet Backbone and Colocation Provider](https://www.he.net/)
* [DNSdumpster.com - dns recon and research, find and lookup dns records](https://dnsdumpster.com/)
* [ViewDNS.info - Your one source for DNS related tools!](https://viewdns.info/)
## WHOIS
* [WHOIS Search, Domain Name, Website, and IP Tools - Who.is](https://who.is/)
## Recon
* [check host http, ping](https://check-host.net/check-http?host=google.com&csrf_token=59af8b32d968249a267ad6bc09684399d27a5d93)
* [ViriBack C2 Tracker](https://tracker.viriback.com/)
* [Shodan Search Engine](https://www.shodan.io/)
* [Online Port Scanner Powered by Nmap | HackerTarget.com](https://hackertarget.com/nmap-online-port-scanner/)
* [Censys Search](https://search.censys.io/)
# Threat Intel
* [AlienVault - Open Threat Exchange](https://otx.alienvault.com/)
* [Robtex](https://www.robtex.com/)
* [ThreatMiner.org | Data Mining for Threat Intelligence](https://www.threatminer.org/)
* [abuse.ch | Fighting malware and botnets](https://abuse.ch/)
* [VirusTotal](https://www.virustotal.com/gui/home/upload)
* [Cisco Talos Intelligence Group - Comprehensive Threat Intelligence](https://talosintelligence.com/)
## Malware Sandbox
* [Cuckoo Sandbox](https://cuckoosandbox.org/)
* [any.run](https://app.any.run/)
* [CAPE Sandbox](https://www.capesandbox.com/)
* [PSafe | Líder no desenvolvimento de aplicativos de segurança, performance e privacidade](https://www.psafe.com/pt-br/)
* [Verficador de malware da Jotti](https://virusscan.jotti.org/pt-br)
## URL analyser
* [Zscaler | Zulu - URL Risk Analyzer](https://zulu.zscaler.com/)
* [hybrid analysis](https://www.hybrid-analysis.com/)
* [URL and website scanner - urlscan.io](https://urlscan.io/)
* [VirusTotal - URL](https://www.virustotal.com/gui/home/url)
## Website Security
* [PageSpeed Insights](https://pagespeed.web.dev/analysis/https-google-com/hjtxthy5of?form_factor=mobile)
* [Website Security Checker | Malware Scan | Sucuri SiteCheck](https://sitecheck.sucuri.net/)
* [ScamAdviser.com | Check a website for risk | Check if fraudulent | Website trust reviews |Check website is fake or a scam](https://www.scamadviser.com/)
# Crypto and hashes
* [The Black Chamber - Letter Frequencies](https://www.simonsingh.net/The_Black_Chamber/letterfrequencies.html)
* [Cyberchef - Defang URL](https://cyberchef.org/#recipe=Defang_URL(true,true,true,'Valid%20domains%20and%20full%20URLs')Defang_IP_Addresses())
* [dCode - Online Ciphers, Solvers, Decoders, Calculators](https://www.dcode.fr/en)
* [CrypTool-Online - CrypTool Portal](https://www.cryptool.org/en/cto/)
* [Base64 Encoder / Decoder Online - AppDevTools](https://appdevtools.com/base64-encoder-decoder)
## Hashing
* [SHAttered](http://shattered.io/)
* [hashes](https://hashes.com/en/decrypt/hash)
* [SRI Hash Generator](https://www.srihash.org/)
* [example_hashes [hashcat wiki]](https://hashcat.net/wiki/doku.php?id=example_hashes)
# misc
## MAC address
* [Home | MAC Vendor Lookup Tool & API | MACVendors.com](https://macvendors.com/)
* https://standards-oui.ieee.org/oui/oui.txt
* [croxyproxy](https://www.croxyproxy.com/)
* [Submarine Cable Map ](https://www.submarinecablemap.com/)
